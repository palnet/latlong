...
class TestEncode(unittest.TestCase):
	...
	def test_not_string(self):
		with self.assertRaises( TypeError ): encodeCoords(0)
	
	def test_incomplete_point(self):
		with self.assertRaises( ValueError ): encodeCoords("0")
		with self.assertRaises( ValueError ): encodeCoords("0 0 0")
	
	def test_invalid_point(self):
		with self.assertRaises( ValueError ): encodeCoords("-91 0")
		with self.assertRaises( ValueError ): encodeCoords("91 0")
		with self.assertRaises( ValueError ): encodeCoords("0 -181")
		with self.assertRaises( ValueError ): encodeCoords("0 181")
...
class TestDecode(unittest.TestCase):
	...
	def test_not_int(self):
		with self.assertRaises( TypeError ): decodeCoords("foo") # string
		with self.assertRaises( TypeError ): decodeCoords(0.0) # float
	
	def test_negative(self):
		with self.assertRaises( ValueError ): decodeCoords(-1)
